...
<div class="row row-sm">
            ...
            <div class="card-body">
                <form action="{{ route('admin.brand.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <p class="mg-b-10 tx-semibold">{{__('cruds.brand_code')}} (<span class="required">*</span>)
                        </p>
                        <input type="text" class="form-control unique_code {{ $errors->has('brand_code') ? 'is-invalid' : '' }}"
                            name="brand_code" id="brand_code" placeholder="{{__('cruds.brand_code')}}" data-model="Brand" data-prefix="BR" data-update_id="0"  required>
                    ...
                    <div class="form-group">
                        <p class="mg-b-10 tx-semibold">{{__('cruds.brand_name')}} (<span class="required">*</span>)
                        </p>
                        <input type="text" class="form-control  {{ $errors->has('brand_name') ? 'is-invalid' : '' }}"
                            name="brand_name" id="brand_name" placeholder="{{__('cruds.brand_name')}}" required>
    ...
    <div class="col-md-8 col-xl-8 col-xs-8 col-sm-8">
        <div class="card border custom-card">
            <div class="card-header custom-card-header">
                <h5 class="main-content-label mb-0">{{ trans('cruds.brand') }}
                    {{ trans('cruds.list') }}</h5>
                <div class="card-options">
                    <a href="javascript:;" class="card-options-collapse py-0" data-bs-toggle="card-collapse"><i
                            class="fe fe-chevron-up"></i></a>
                    ...
                    <table class=" table table-bordered table-striped table-hover datatable datatable-Income">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('cruds.brand_code') }}</th>
                                <th>{{ trans('cruds.brand_name') }}</th>
                            ...
                            <tr data-entry-id="{{ $brand->id }}">
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-xs btn-info editAction" href="javascript:void(0)"
                                        data-table="Brand" data-column="id" data-value="{{$brand->id}}">
...
<script>
$(function() {
    let route = "{{ route('admin.inventory_management.brand.massDestroy') }}";
    let hasPermission = 0;
    <?php if (delete()) {?>
    hasPermission = 1;
    <?php }?>
