...
<div class="row row-sm">
            ...
            <div class="card-body">
                <form action="{{ route('admin.designation.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <p class="mg-b-10 tx-semibold">{{__('cruds.designation_code')}} (<span class="required">*</span>)
                        </p>
                        <input type="text" class="form-control unique_code {{ $errors->has('designation_code') ? 'is-invalid' : '' }}"
                            name="designation_code" id="designation_code" placeholder="{{__('cruds.designation_code')}}" data-model="Designation" data-prefix="DC" data-update_id="0"  required>
                        @if($errors->has('designation_code'))
                        <em class="invalid-feedback">
                            {{ $errors->first('designation_code') }}
                        </em>
                        @endif
                    </div>
                    <div class="form-group">
                        <p class="mg-b-10 tx-semibold">{{__('cruds.designation_name')}} (<span class="required">*</span>)
                        </p>
                        <input type="text" class="form-control  {{ $errors->has('designation_name') ? 'is-invalid' : '' }}"
                            name="designation_name" id="designation_name" placeholder="{{__('cruds.designation_name')}}" required>
    ...
    <div class="col-md-8 col-xl-8 col-xs-8 col-sm-8">
        <div class="card border custom-card">
            <div class="card-header custom-card-header">
                <h5 class="main-content-label mb-0">{{ trans('cruds.designation') }}
                    {{ trans('cruds.list') }}</h5>
                <div class="card-options">
                    <a href="javascript:;" class="card-options-collapse py-0" data-bs-toggle="card-collapse"><i
                            class="fe fe-chevron-up"></i></a>
                    ...
                    <table class=" table table-bordered table-striped table-hover datatable datatable-Income">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('cruds.designation_code') }}</th>
                                <th>{{ trans('cruds.designation_name') }}</th>
                            ...
                            <tr data-entry-id="{{ $designation->id }}">
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.designation.show', $designation->id) }}">
                                    ...
                                    <a class="btn btn-xs btn-info editAction" href="javascript:void(0)"
                                        data-table="Designation" data-column="id" data-value="{{$designation->id}}">
...
<script>
$(function() {
    let route = "{{ route('admin.general_settings.designation.massDestroy') }}";
    let hasPermission = 0;
    <?php if (delete()) {?>
    hasPermission = 1;
    <?php }?>
